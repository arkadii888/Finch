...
- **Agent**: This is the Python orchestrator for an autonomous drone stack using Gemma 4 E2B. It turns natural-language operator prompts into validated flight missions, uploads them to the vehicle controller over gRPC, and optionally runs onboard vision (ArduCam + Hailo YOLO) with an automatic person-rescue path. 
...
To try out the agent for mission planning without a drone see the quick start [here](https://github.com/arkadii888/Agent/blob/main/README.md).
