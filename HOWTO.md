...
Regarding power, we initially used a power bank, but it made the drone too heavy. Because of this, we connected the drone's main battery to both the drone itself and the Raspberry Pi using [adapter](https://www.amazon.de/-/en/gp/aw/d/B0F9FLF6QB?psc=1&ref=ppx_pop_mob_b_asin_title). This requires some soldering (although it is not complicated), so we will leave this decision up to you. 
